...
/*
there is a conflict between std::string_view and pmem::obj::string_view, which yields the following error:
    undefined reference to `KVDK_NAMESPACE::Engine::Open(std::basic_string_view<char, std::char_traits<char> >, KVDK_NAMESPACE::Engine**, 
KVDK_NAMESPACE::Configs const&, _IO_FILE*)'
    collect2: error: ld returned 1 exit status
The issue is that when __cpp_lib_string_view is defined (which happens with C++17), the library uses std::string_view instead of its own implementation,
causing the mismatch we're seeing. By undefining __cpp_lib_string_view, we force it to use pmem's implementation which is what the library was compiled with.
We could also try compiling with C++11 or C++14 where std::string_view isn't available: -std=c++11 or -std=c++14
*/
    ...
    if (!options.empty()) {
        ...
        for (auto &op : ops) {
            ...
            if (kv.front() == "max_access_threads") {
                ...
                kvdk_configs.pmem_file_size = std::stoull(kv.back());
                ...
                kvdk_configs.pmem_segment_blocks = std::stoull(kv.back());
...
int KVDKStore::submit_transaction(KeyValueDB::Transaction t) {
    ...
    // TODO: batch write not implemented because of the MERGE operation not supported by kvdk
